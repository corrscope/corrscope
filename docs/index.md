...
For each candidate trigger point (a local maximum of the buffer/edge locator), we compute the quality by summing  `Edge Strength` * "slope around the point" + `Buffer Strength` * "similarity with buffer" (measuring alignment with previous frame). Then we use the edge/correlation peak with the highest slope/correlation score.
