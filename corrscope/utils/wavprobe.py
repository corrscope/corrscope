...
@attr.dataclass
class FmtChunk:
    size: int  # size of format subchunk in bytes (minus 8 for "fmt " and itself)

    format_tag: int  # PCM, float, or compressed format
    channels: int  # number of channels
    fs: int  # sampling frequency in samples per second

    bytes_per_second: int  # overall byte rate for the file
    block_align: int  # bytes per sample, including all channels
    bit_depth: int  # bits per sample


def _read_fmt_chunk(fid, is_big_endian) -> FmtChunk:
    """
    Notes
    -----
    ...
    return FmtChunk(
        size, format_tag, channels, fs, bytes_per_second, block_align, bit_depth
    )
...
def probe(filename: str) -> FmtChunk:
    """
    Open a WAV file and return sample/channel format metadata.
